...
const loginviaform= createAsyncThunk(
    ...
    async ({email,password,rememberMe}, { rejectWithValue }) => {
        try {
            ...
            const res = await axios.post(import .meta.env.VITE_BACKEND+"/auth/loginviaform", { email, password })
            if (res.status === 200) {
                if (rememberMe) {
...
const veriyUser= createAsyncThunk("/auth/verifyUser", async (token, { rejectWithValue }) => {
    try {
        const res = await axios.get(import .meta.env.VITE_BACKEND+"/auth/checktoken/"+  token)
