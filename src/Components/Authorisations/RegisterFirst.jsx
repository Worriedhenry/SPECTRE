...
const Register = ({ setActiveTabs, username, setUsername, fullname, setFullname, phone, setPhone, email, setEmail }) => {
    ...
    const handleEmailVerification = async () => {
        if (email) {
            ...
            const res = await axios.get((import .meta.env.VITE_BACKEND_GATEWAY+"/auth" ||import .meta.env.VITE_BACKEND)+"/auth/emailexist/" + email)
    ...
    const handleUsernameVerification = async () => {
        if (username) {
            const res = await axios.get((import .meta.env.VITE_BACKEND_GATEWAY+"/auth" ||import .meta.env.VITE_BACKEND)+"/auth/usernameexist/" + username)
    ...
    const handlePhoneVerification = async () => {
        if (phone) {
            ...
            const res = await axios.get((import .meta.env.VITE_BACKEND_GATEWAY+"/auth" ||import .meta.env.VITE_BACKEND)+"/auth/phoneexist/" + phone)
            if (res.data) {
